...
function query(result: { data?: unknown; error?: unknown }) {
  const builder = {
    select: vi.fn(), insert: vi.fn(), update: vi.fn(), eq: vi.fn(), order: vi.fn(), single: vi.fn(), maybeSingle: vi.fn(),
    then: (resolve: (value: typeof result) => unknown) => Promise.resolve(result).then(resolve),
  };
  for (const method of [builder.select, builder.insert, builder.update, builder.eq, builder.order, builder.single, builder.maybeSingle]) method.mockReturnValue(builder);
  return builder;
}
...
describe("meeting knowledge repository", () => {
  ...
  it("uses lease-owner filters when completing or failing a job", async () => {
